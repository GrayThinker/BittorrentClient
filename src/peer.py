...
class Peer:
    ...
    async def connect(self):
        ...
        handshake = unpack('>B19s8x20s20s', resp)
        
        if handshake[2] != self.info_hash:
            log.info(f"{self} gave non-matching info-hash")
            writer.close()
            await writer.wait_closed()
            return False
        
        self.peers_own_id = handshake[3]
        self.reader = reader
        self.writer = writer
    ...
    async def download(self):
        ...
        if msg.code == Message.choke:
            self._is_choking = 1

        elif msg.code == Message.unChoke:
            self._is_choking = 0

        elif msg.code == Message.interested:
            self._is_interested = 1
        
        elif msg.code == Message.notInterested:
            self._is_interested = 0
        
        elif msg.code == Message.have:
            piece_index = msg.index
    
        elif msg.code == Message.bitField:
            self.bitField = BitArray(msg.data)

        elif msg.code == Message.request:
            # self.writer.write(request_piece())
            pass

        elif msg.code == Message.piece:
            # file_saver(msg.index, msg.begin, msg.block)
            pass
    ...
    def __init__(self, raw_bytes):
        ...
        if self.length == 0:
            ...
            if self.code == have:
                ...
                self.data = raw_bytes[5:self.length+4]
